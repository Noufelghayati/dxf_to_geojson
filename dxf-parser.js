...
DxfParser.prototype._parse = function(dxfString) {
	...
	var parseMTEXT = function() {
		...
		while(curr !== 'EOF') {
			...
			switch(curr.code) {
				...
				case 72:
					...
					break;
				case 11:
					entity.directionX = curr.value;
					curr = scanner.next();
					break;
				case 21:
					entity.directionY = curr.value;
					curr = scanner.next();
					break;
				case 31:
					entity.directionZ = curr.value;
					curr = scanner.next();
					break;
				default:
					checkCommonEntityProperties(entity);
		...
		if (entity.directionX !== undefined && entity.directionY !== undefined) {
			...
			entity.mtextOrientation = Math.atan2(entity.directionY, entity.directionX) * (180 / Math.PI);
		} else if (entity.rotation !== undefined) {
			// Use rotation angle if direction vector is not provided
			entity.mtextOrientation = entity.rotation;
		} else {
			entity.mtextOrientation = 0; // Default to 0 if neither is provided
		}
	...
	var parseHATCH = function() {
		var entity = { type: 'HATCH', edges: [] };
		...
		while (curr !== 'EOF') {
			...
			switch (curr.code) {
				case 10:
					// Start a new edge with the x value
					currentEdge = { x: curr.value };
					break;
				case 20:
					// Complete the edge with the y value and add it to edges if currentEdge exists
					if (currentEdge) {
						currentEdge.y = curr.value;
						// Only add the edge if x and y are not both zero
						if (!(currentEdge.x === 0 && currentEdge.y === 0)) {
							entity.edges.push(currentEdge);
						}
						currentEdge = null; // Reset currentEdge after adding to edges
					}
					break;
				case 62:
					entity.colorIndex = curr.value;
				...
				case 70:
					...
					entity.patternFill = (curr.value & 2) !== 0;
					break;
				case 71:
					entity.associative = curr.value !== 0;
					break;
				case 72:
					entity.style = curr.value;
					break;
				case 91:
					entity.numBoundaryPaths = curr.value;
					break;
				case 93:
					entity.numEdges = curr.value;
					break;
				default:
					// Process other codes and add them to the entity if necessary
					if (curr.code >= 100 && curr.code <= 107) {
						entity[curr.code] = curr.value;
					}
					break;
			}
			curr = scanner.next();
